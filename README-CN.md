...
我是个兴趣使然的桌面应用开发者🤓，利用搬砖之余开发了 HKU。

HKU 是一款全能的下载工具，支持下载 HTTP、FTP、BT、磁力链等资源。它的界面简洁易用，希望大家喜欢 👻。

✈️ 去 [官网](https://hku.app/zh-CN) 逛逛  |  📖 查看 [帮助手册](http://hku.app/support/issues)

## 💽 安装稳定版

[GitHub](https://github.com/agalwood/HKU/releases) 和 [官网](https://hku.app/zh-CN) 提供了已经编译好的稳定版安装包，当然你也可以自己克隆代码编译打包。

### Windows

建议使用安装包（HKU-Setup-x.y.z.exe）安装 HKU 以确保完整的体验，例如关联 torrent 文件，捕获磁力链等。

如果你在 Windows 是用包管理工具来管理应用，如 [Chocolatey](https://chocolatey.org)、[scoop](https://github.com/lukesampson/scoop)，你可以使用它们安装 HKU。

#### Chocolatey
感谢 [@Yato](https://github.com/iYato) 持续维护着 [HKU Chocolatey](https://community.chocolatey.org/packages/motrix) 包。要安装 HKU，请从 `命令行` 或 `PowerShell` 中运行以下命令：
...
#### scoop
如果你更喜欢便携版，你可以使用 [scoop](https://github.com/lukesampson/scoop)（需要 Windows 7+，天朝用户可能需要设置 Git 代理）安装最新便携版本的 HKU。
...
macOS 用户可以使用 `brew` 安装 HKU，感谢 [@Mitscherlich](https://github.com/Mitscherlich) 的 [PR](https://github.com/Homebrew/homebrew-cask/pull/59494)。
...
#### 自动更新
HKU v1.8.0+ 版本更改了应用 BundleID ( `net.agalwood.HKU` => `app.motrix.native` ), HKU v1.6.11 的自动更新会因为签名不一致而失败。[HKU 安装助手](https://github.com/motrixapp/motrix-install-assistant)将帮助您安装最新的 HKU 应用程序。
...
你可以下载 `AppImage` （适用于所有 Linux 发行版）或 `snap` 来安装 HKU，更多 Linux 安装包格式请查看 [GitHub/release](https://github.com/agalwood/HKU/releases) 。

HKU 在 Linux 中首次启动可能需要使用 `sudo` 运行，因为可能没有创建下载会话文件的权限 (`/var/cache/aria2.session`)。

如果你想自己通过编译源码来安装，请阅读 **编译打包** 部分。

#### AppImage
最新版的 HKU AppImage 需要自己手动进执行桌面集成。请查看 [AppImageLauncher](https://github.com/TheAssassin/AppImageLauncher) 的文档进行操作。
...
Deepin 20 Beta 用户安装 HKU 失败的问题，请按照以下方法处理：

打开`终端`，黏贴运行如下命令之后再次安装 HKU。
```bash
sudo apt --fix-broken install
```

#### Snap
HKU 已经上架 [Snapcraft](https://snapcraft.io/motrix) ，Ubuntu 用户推荐从 Snap 商店下载。
...
#### AUR
对于 Arch Linux 用户，可以使用 [aur](https://aur.archlinux.org/packages/motrix/) 安装 HKU，感谢维护者 [@weearc](https://github.com/weearc)。
...
#### Flatpak
感谢 [@proletarius101](https://github.com/proletarius101) 的 [PR](https://github.com/flathub/flathub/pull/2334)，HKU 已经上架 [Flathub](https://flathub.org/apps/details/net.agalwood.HKU)，喜欢 Flatpak 的 Linux 用户可以尝试。
...
欢迎大家将 HKU 翻译成更多的语言版本 🧐，开工之前请先阅读一下 [翻译指南](./CONTRIBUTING-CN.md#-翻译指南)。
