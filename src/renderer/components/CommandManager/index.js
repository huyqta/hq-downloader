...
export default class CommandManager extends EventEmitter {
  ...
  register (id, fn) {
    if (this.commands[id]) {
      console.log('[HKU] Attempting to register an already-registered command: ' + id)
      return null
    }
    if (!id || !fn) {
      console.error('[HKU] Attempting to register a command with a missing id, or command function.')
      return null
    }
    this.commands[id] = fn
