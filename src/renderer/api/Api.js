...
export default class Api {
  ...
  savePreferenceToNativeStore (params = {}) {
    ...
    if (!isEmpty(user)) {
      console.info('[HKU] save user config: ', user)
      config.user = user
    }

    if (!isEmpty(system)) {
      console.info('[HKU] save system config: ', system)
      config.system = system
      this.updateActiveTaskOption(system)
    }

    if (!isEmpty(others)) {
      console.info('[HKU] save config found illegal key: ', others)
    }
  ...
  fetchDownloadingTaskList (params = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        console.log('[HKU] fetch downloading task list data:', data)
        const result = mergeTaskResult(data)
        resolve(result)
      }).catch((err) => {
        console.log('[HKU] fetch downloading task list fail:', err)
        reject(err)
      })
    })
  }
  ...
  fetchTaskItemWithPeers (params = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        console.log('[HKU] fetchTaskItemWithPeers:', data)
        const result = data[0] && data[0][0]
        const peers = data[1] && data[1][0]
        result.peers = peers || []
        console.log('[HKU] fetchTaskItemWithPeers.result:', result)
        console.log('[HKU] fetchTaskItemWithPeers.peers:', peers)
