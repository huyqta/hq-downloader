...
export const showItemInFolder = (fullPath, { errorMsg }) => {
  ...
  access(fullPath, constants.F_OK, (err) => {
    console.warn(`[HKU] ${fullPath} ${err ? 'does not exist' : 'exists'}`)
    if (err && errorMsg) {
...
export const moveTaskFilesToTrash = (task) => {
  ...
  access(path, constants.F_OK, async (err) => {
    console.log(`[HKU] ${path} ${err ? 'does not exist' : 'exists'}`)
    if (!err) {
  ...
  access(extraFilePath, constants.F_OK, async (err) => {
    console.log(`[HKU] ${extraFilePath} ${err ? 'does not exist' : 'exists'}`)
    if (!err) {
