...
export default class Application extends EventEmitter {
  ...
  offConfigListeners() {
    try {
      ...
      logger.warn('[HKU] offConfigListeners===>', e)
    }
    this.configListeners = {}
  }
  ...
  setupLogger() {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      logger.info(`[HKU] detected ${key} value change event:`, newValue, oldValue)
  ...
  async stopEngine() {
    ...
    try {
      ...
      logger.info('[HKU] stopEngine.setImmediate===>')
      setImmediate(() => {
        this.engine.stop()
      })
    } catch (err) {
      logger.warn('[HKU] shutdown engine fail: ', err.message)
    } finally {
      // no finally
    }
  }
  ...
  watchProtocolsChange() {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      ...
      logger.info('[HKU] setup protocols client:', newValue)
  ...
  async startUPnPMapping() {
    ...
    try {
      ...
      logger.warn('[HKU] start UPnP mapping fail', e.message)
    }
  }
  ...
  async stopUPnPMapping() {
    ...
    try {
      ...
      logger.warn('[HKU] stop UPnP mapping fail', e)
    }
  }
  ...
  watchUPnPPortsChange() {
    ...
    watchKeys.forEach((key) => {
      this.configListeners[key] = systemConfig.onDidChange(key, async (newValue, oldValue) => {
        logger.info('[HKU] detected port change event:', key, newValue, oldValue)
        ...
        try {
          ...
          logger.info('[HKU] change UPnP port mapping failed:', e)
        }
      })
    })
  }
  ...
  watchUPnPEnabledChange() {
    ...
    this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
      logger.info('[HKU] detected enable-upnp value change event:', newValue, oldValue)
      if (newValue) {
        this.startUPnPMapping()
      } else {
  ...
  syncTrackers(source, proxy) {
    ...
    setTimeout(() => {
      fetchBtTrackerFromSource(source, proxy).then((data) => {
        logger.warn('[HKU] auto sync tracker data:', data)
        ...
        logger.warn('[HKU] auto sync tracker failed:', err.message)
      })
    }, 500)
  }
  ...
  autoSyncTrackers() {
    ...
    logger.info('[HKU] auto sync tracker checkIsNeedRun:', result)
    if (!result) {
      return
    }
  ...
  stop() {
    try {
      ...
      logger.warn('[HKU] stop error: ', err.message)
    }
  }
  ...
  handleFile(filePath) {
    ...
    readFile(filePath, (err, data) => {
      if (err) {
        logger.warn(`[HKU] read file error: ${filePath}`, err.message)
        return
      }
  ...
  async resetSession() {
    ...
    setTimeout(() => {
      unlink(sessionPath, (err) => {
        logger.info('[HKU] Removed the download seesion file:', err)
      })
  ...
  savePreference(config = {}) {
    ...
    if (!isEmpty(system)) {
      console.info('[HKU] main save system config: ', system)
      this.configManager.setSystemConfig(system)
      this.engineClient.changeGlobalOption(system)
    }

    if (!isEmpty(user)) {
      console.info('[HKU] main save user config: ', user)
  ...
  handleCommands() {
    ...
    this.on('application:setup-protocols-client', (protocols) => {
      ...
      logger.info('[HKU] setup protocols client:', protocols)
    ...
    this.on('application:reveal-in-folder', (data) => {
      const { gid, path } = data
      logger.info('[HKU] application:reveal-in-folder===>', path)
      if (path) {
  ...
  handleIpcMessages() {
    ...
    ipcMain.on('event', (event, eventName, ...args) => {
      logger.log('[HKU] ipc receive event', eventName, ...args)
