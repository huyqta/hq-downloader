...
export default class Engine {
  ...
  start () {
    ...
    logger.info('[HKU] Engie pid path:', pidPath)
    ...
    this.instance.once('close', () => {
      try {
        unlink(pidPath, (err) => {
          if (err) {
            logger.warn(`[HKU] Unlink engine process pid file failed: ${err}`)
          }
        })
      } catch (err) {
        logger.warn(`[HKU] Unlink engine process pid file failed: ${err}`)
    ...
    if (is.dev()) {
      this.instance.stdout.on('data', (data) => {
        logger.log('[HKU] engine stdout===>', data.toString())
      })

      this.instance.stderr.on('data', (data) => {
        logger.log('[HKU] engine stderr===>', data.toString())
      })
    }
  }

  stop () {
    logger.info('[HKU] engine.stop.instance')
    if (this.instance) {
      this.instance.kill()
      this.instance = null
    }
  ...
  writePidFile (pidPath, pid) {
    writeFile(pidPath, pid, (err) => {
      if (err) {
        logger.error(`[HKU] Write engine process pid failed: ${err}`)
      }
    })
  }
  ...
  getEngineBinPath () {
    ...
    if (!binIsExist) {
      logger.error('[HKU] engine bin is not exist:', result)
