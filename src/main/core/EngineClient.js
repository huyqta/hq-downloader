...
export default class EngineClient {
  ...
  connect () {
    logger.info('[HKU] main engine client connect', this.options)
  ...
  async call (method, ...args) {
    return this.client.call(method, ...args).catch((err) => {
      logger.warn('[HKU] call client fail:', err.message)
    })
  }

  async changeGlobalOption (options) {
    logger.info('[HKU] change engine global option:', options)
